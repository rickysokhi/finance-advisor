( function ( document, $, undefined ) {
	...
	$( '#finance-sections-importexport-toggles' ).on( 'click', '.finance-importexport-toggle', function(e) {
		e.preventDefault();
		var $this = $(this),
			el    = $this.attr( 'href' );
		$( '.finance-importexport-toggle' ).toggleClass( 'active' );
		$( '.finance-sections-importexport-content' ).not( el ).hide();
		$( el ).show();
	});

	var $archiveMetabox  = $( '.finance-content-archive-metabox' ),
		$removeLoop      = $( '#remove_loop' ),
		$settingsEnabled = $( '#enable_content_archive_settings' ),
		$settingWrap     = $( '.finance-archive-setting-wrap' ),
		$settingsWrap    = $( '.finance-archive-settings-wrap' );
	...
	var $sectionsRepeater = $( '#finance_sections_repeat' );

	if ( $sectionsRepeater.length ) {

		// Get the sections
		var $sections = $( '.finance-section' );
	...
	function _sectionSettings( $section ) {
		...
		var $button = $section.find( '.finance-section-settings-toggle' );

		_toggleAria( $button, 'aria-pressed' );
		_toggleAria( $button, 'aria-expanded' );

		$section.on( 'click', '.finance-section-settings-toggle', function(e) {

			e.preventDefault();

			$section.toggleClass( 'finance-settings-open' );
		...
		$section.on( 'click', '.finance-section-content', function(e) {
			e.preventDefault();
			$section.removeClass( 'finance-settings-open' );
		});

		$section.on( 'click', '.finance-section-settings-close', function(e) {
			e.preventDefault();
			$section.removeClass( 'finance-settings-open' );
		});
